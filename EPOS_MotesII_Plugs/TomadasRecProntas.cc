...
public:
	...
	*/
	...
	/**
	* Metodo responsavel por receber uma mensagem via USB/UART.
	*/
	static int receberViaUART();
...
private:
	...
	NIC nic; ;			/**< Instancia do módulo de rádio */
...
public:
	...
	*/
	void setPowerSavingLimit(int limit){
		limite = limit;
	}
...
/**
	Metodo sem retorno para receber comandos UART
	@param inforTomadas msg
*/
void receberUART(char cmd[]){
	int templimit = 0;
	cout<<"UART\n";
	if (cmd[0] == 'S'){
		cout<<"UARTI\n";
		templimit += 10000 * cmd[1];
		templimit += 1000 * cmd[2];
		tomada.setPowerSavingLimit(templimit);
		imprimirArray();
		return;
	}
	...
	if(cmd[0] == 'L'){
		if(cmd[1] == 'T'){
			tomada.setPowerSavingMode(true);
		} else
		if(cmd[1] == 'F'){
			tomada.setPowerSavingMode(false);
		}
		cout<<"UARTE\n";
		return;
	}
...
void imprimirProprio(){
	...
	cout<<"PWSL: "<<tomada.getPowerSavingLimit()<<"\n";
}
...
void tomadaInteligente(){
	...
	if(tomada.getPowerSavingMode() && previsaoGeral > tomada.getPowerSavingLimit()){
		for(int i = 0; i < numTomadasExternas;i++){
			if(tomadasExternas[i].prioridade < tomada.getPrioridade()){
				if(tomadasExternas[i].tipo == ATUALIZACAO){
					myTurn = false;
					imprimirProprio();
					break;
				} 
...
/**
* Metodo com retorno inteiro para receber mensagens via UART
* @return: retorna 0 ao ser encerrado
*/
int Mensageiro::receberViaUART(){
	char cmd[6];
...
Mensageiro::Mensageiro(Gerente * gnt){
		...
		//thread1 =  new Thread( &receberViaUART );
}
